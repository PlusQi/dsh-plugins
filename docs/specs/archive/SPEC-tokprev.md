# tokprev - 设计定稿（单轮 token 消耗：事前预告 + 事后实报）

- **状态**：已定稿（grilling 会话共识，2026-08-26）
- **归属**：`dsh-plugins` 扩展集首成员（`lib/client.js` 的 `PLUGINS.tokprev` 注册块 + `cordis.patch.yml` 的 `id: tokprev` 行，经 `config.plugin` 激活）
...
| 修订 3 | 命名口径 | **名字跟随主打组件（预告）**；功能定义为"事前预告 + 事后实报"，不拆两插件 | 徽标与预告的输出区间同源（按轮 `usage` 分组），拆分割裂闭环；`tokmeter` 类与内置 `token-meter` 撞名 |
