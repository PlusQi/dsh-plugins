window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Pack-owned styles; self-managed tag with idempotent insert and fiber-scoped removal. */
		const CSS = [
		...
		function ensureStyles() {
			...
			const selector = 'style[data-plugin-css="dsh-plugins/styles"]';
			let tag = document.querySelector(selector);
			if (tag === null) {
				tag = document.createElement("style");
				tag.dataset.pluginCss = "dsh-plugins/styles";
				tag.textContent = CSS;
		...
		// ── plugin: tokprev ────────────────────────────────────────────────────────
		// 设计定稿与决策记录见 SPEC-tokprev.md。新增插件：在此追加各自的
		// helpers + 组件 + slots.inject 注册块，并在 cordis.patch.yml 加一行。
		...
		const inject = ["slots"];
		/**
		 * Browser plugin body: one slots.inject block per pack plugin.
		 * - tokprev / conversation.composer.dock (list): the pre-send prediction line.
		 * - tokprev / conversation.chat.assistant-actions (list): per-turn real-usage
		 *   badge, rendered only on the closing assistant message of a completed turn.
		 */
		function apply(ctx) {
			const disposeStyles = ensureStyles();
			ctx.effect(() => () => disposeStyles(), "dsh-plugins: styles");
