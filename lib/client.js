window.__ModuleLoader__.load({
	...
	function TokstatsButton({ wide, useSessions, t }) {
		...
		function PromptoptPopup({ snapshot, phase, result, onAdopt, onClose, t }) {
			const ready = phase === "ready" && result !== null && result !== undefined;
			return react.createElement("div", { className: "dsh-promptopt-panel" },
		...
		function PromptoptButton({ useInput, inputActions, t }) {
			...
			return react.createElement("div", { className: "dsh-promptopt-root" },
				react.createElement("button", {
					...
					react.createElement("span", { className: "dsh-promptopt-triggerLabel" }, t("button.label"))),
				open
					? react.createElement(PromptoptPopup, {
						snapshot: snapshot, phase: phase, result: result, onAdopt: adopt, onClose: close, t: t
					})
					: null);
		}

		const promptoptCss = [
			".dsh-promptopt-root{position:relative;display:inline-flex;align-items:center}",
			".dsh-promptopt-trigger{color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:8px;align-items:center;gap:5px;padding:0 7px;height:24px;font:inherit;font-size:12px;display:inline-flex}",
			...
			".dsh-promptopt-triggerLabel{color:inherit;white-space:nowrap}",
			".dsh-promptopt-panel{z-index:30;box-sizing:border-box;border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);width:420px;max-width:calc(100vw - 24px);max-height:60vh;box-shadow:var(--dsw-shadow-lv3);color:var(--dsw-alias-label-secondary);border-radius:12px;flex-direction:column;display:flex;position:absolute;bottom:calc(100% + 8px);left:0;overflow:hidden;font-size:12px;line-height:20px}",
		...
		/**
		 * promptopt 注册体：conversation.composer.dock（list，session scope）
		 * 按钮 + 弹层。order 30 避开 tokprev 的 20；同屏拥挤度按 SPEC §3.1 目检，
		 * 过挤则挪座位。
		 */
		function promptoptApply(ctx) {
			promptoptCtx = ctx;
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register(
				{ name: "conversation.composer.dock", id: "promptopt-button", order: 30, locale: LOCALE_NS_PROMPTOPT },
				PromptoptButton
			));
		}
